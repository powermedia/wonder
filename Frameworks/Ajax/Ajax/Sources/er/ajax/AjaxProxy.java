 ...
 * <code>proxyName</code>. By default, it will configure the parent component as
 * one java proxy object and name it <code>wopage</code> from the javascript
 * side. A JSONRPCBridge object is created if not given as a binding. If the
 * binding is there but the value is null, it will create the bridge then push
 * it to the binding. That way, you can configure a single bridge for multiple
 * proxy objects. It is of good practice to provide a value in a binding (at
 * least a binding) so that the object is not created on every ajax request.
 * </p>
 * <p>
 * The <em>proxy</em> object will be the one visible for RPC from the javascript
 * world. For example, the following binding:<br>
 * <code>
 ...
 * </p>
 * <p>
 * Remember that is no proxy object is given, it will use the parent component,
 * which is the component in which this component is embeded.
 * </p>
 * <h2>Todo</h2>
 * <ul>
 * <li>Complete the JSON-RPC integration to be able to leverage all
 * possibilities of that library (foreign references, etc.).
 * </ul>
 * 
 * @binding proxy Server side object (Java) that will be visible for rpc
 *          communication (Javascript). If no object is bound, the parent()
 *          object is assigned by default.
 * @binding proxyName Client side name (Javascript) used to identify the proxy
 *          (Java) from the bridge object.
 * @binding name Client side name (Javascript) of the bridge object.
 * @binding JSONRPCBridge Server side object (Java) used to handle the request.
 *          Of no value are bound, a new object is created for every ajax
 *          request. If a binding is there but null value, a new object will be
 *          created and pushed to the binding so that this new object can be
 *          shared for multiple proxy.
 * @binding lazy (default false) if true, the proxy is only initialized
 *          on-demand, rather than on-load
 * 
 * @author Jean-François Veillette <jfveillette@os.ca>
 * @version $Revision $, $Date $ <br>
 *          &copy; 2005-2006 OS communications informatiques, inc.
 *          http://www.os.ca Tous droits réservés.
 */
public class AjaxProxy extends AjaxComponent {
	...
	private JSONRPCBridge getBridge() {
		...
		bridge.registerObject(proxyName, getProxy());
		return bridge;
	}
	...
	public WOActionResults handleRequest(WORequest request, WOContext context) {
		...
		try {
			...
			output = getBridge().call(new Object[] { request, context, response, getProxy() }, input);
		}
