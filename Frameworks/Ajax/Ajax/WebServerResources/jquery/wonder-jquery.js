...
var AjaxSubmitButton = {
	...
	observeField : function(updateContainerID, formFieldID, observeFieldFrequency, partial, observeDelay, options) {
		...
		if ($formField.length > 0 && ($formField.is("input") || $formField.is("select") || $formField.is("textarea"))) {
			...
			defer(function() {
				var field = withId(formFieldID);
				if (field) {
					if (observeFieldFrequency == null) {
						if (field.type.toLowerCase() == 'radio') {
							new Form.Element.RadioButtonObserver(field, submitFunction);
						} else {
							new Form.Element.EventObserver(field, submitFunction);
						}
					} else {
						notImplemented("Element.Observer");
						new Form.Element.Observer(field, observeFieldFrequency, submitFunction);
					}
				}
			});
		}
